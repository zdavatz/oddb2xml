    ...
    def prepare_products
      unless @products
        ...
        @refdata.each_pair do |ean13, item|
          next if item && item.is_a?(Hash) && item[:atc_code] && /^Q/i.match(item[:atc_code])
        ...
        xml.PRODUCT(XML_OPTIONS) {
          ...
          @products.sort.to_h.each do |ean13, obj|
            ...
            xml.PRD("DT" => obj[:last_change]) do
              nbr_products += 1
              xml.GTIN ean
              ppac = ((_ppac = @packs[ean.to_s[4..11]]) && !_ppac[:is_tier] ? _ppac : {})
      ...
      _builder = Nokogiri::XML::Builder.new(encoding: "utf-8") do |xml|
        ...
        xml.ARTICLE(XML_OPTIONS) {
          ...
          @articles.each do |obj|
            ...
            if no8
              ppac = ((_ppac = pack_info) && !_ppac[:is_tier] ? _ppac : nil)
            end
            zur_rose = nil
            ...
            xml.ART("DT" => obj[:last_change] || "") do
              ...
              # xml.EXP
              if false
                if item && item[:substances] && (substance = item[:substances].first)
