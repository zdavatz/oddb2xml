        ...
        composition.substances.each do |substance_item|
          ...
          substance_item.is_active_agent = (active_agents.find { |x| 
                                              /#{x.downcase
                                                                ...
                                                                .gsub(')', '\)')
                                                                .gsub('[', '\[')
                                                                .gsub(']', '\]')}($|\s)/.match(active_substance_name)
}                                             != nil)
...
class ParseComposition
  ...
  def self.from_string(string)
    ...
    puts "ParseComposition.new cleaned #{cleaned}" if VERBOSE_MESSAGES and !cleaned.eql?(stripped)
    ...
    if result.excipiens and result.excipiens.unit
      ...
      result.substances.each { |substance|
        next unless substance.is_a?(ParseSubstance)
            ...
            substance.dose.unit = "#{substance.dose.unit}#{pro_qty}" if substance.unit and !substance.unit.eql?(result.excipiens.unit)
      }
    end
    ...
    result.corresp = ast[:corresp].to_s.sub(/:\s+/, '') if !result.corresp and ast.is_a?(Hash) and ast[:corresp]
    result
  end
end
